...
(function($) {
	...
	function hideTreeNode(node) {
		if (!node.isRoot) {
			$("#" + node.tId).hide();
		}
	}

    function filterTree(startNode, pattern) {
	var thisNodeCounter = 0;

	if (startNode.isParent) {
	    var n = startNode.nodes;
            var res = 0;
	    for (var i=0; i < n.length; i+=1) {
		res += filterTree(n[i], pattern);
            }
		...
		hideTreeNode(startNode);
	    }
	} else {
	    if (startNode.name.match(pattern) != null
                || (startNode.comment
                    && startNode.comment.match(pattern) != null)) {
	...
	function zTreePlugin(){
		return {
			...
			hideNode: function(treeNode) {
				if (!treeNode) return;
				hideTreeNode(treeNode);
			},

			showNode: function(treeNode) {
				if (!treeNode) return;
				showTreeNode(treeNode);
			},

			filter: function(text) {
				if (this.setting.previousFilterText != "" && text == "") {
					expandCollapseSonNode(this.setting, null, false, false);
				}

				filterTree(this.setting.root, new RegExp(".*" + text + ".*", "i"));

				if (this.setting.previousFilterText == "" && text != "") {
					expandCollapseSonNode(this.setting, null, true, false);
				}

				this.setting.previousFilterText = text;
			}
